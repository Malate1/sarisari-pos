  ...
  useEffect(() => {
    ...
    const initializeScanner = () => {
      try {
        ...
        // Start rendering the camera element
        scanner.render(
          (decodedText, decodedResult) => {
            // Success callback: play success sound/effect
            playSuccessFeedback();
            // Send scanned text back to the parent component
            if (onScanSuccess) onScanSuccess(decodedText, decodedResult);
            // Optional: auto-close after successful scan
            setTimeout(() => {
              if (onClose) onClose();
            }, 500);
          },
          (errorMessage) => {
            // Failure callback: triggered continuously while looking for codes
            // Don't log every error to avoid console spam
            if (errorMessage.includes("No MultiFormat Readers")) {
              setErrorMessage("Position the barcode clearly in frame");
            }
