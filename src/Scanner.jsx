	...
	useEffect(() => {
		...
		const checkCameraPermission = async () => {
			try {
				...
				setErrorMessage(
					"Camera access denied. Please enable camera permissions to use the scanner.",
				);
		...
		const initializeScanner = async () => {
			try {
				...
				scanner.render(
					...
					backCamera.id, // 👈 FORCE BACK CAMERA HERE
				);
						...
						<div className="flex items-start gap-2">
							...
							<div>
								...
								<p className="text-xs text-gray-600 mt-0.5">
									...
									<br />• Try a different barcode if scanning fails
								</p>
							</div>
						</div>
					</div>
