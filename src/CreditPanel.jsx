		...
		if (result.isConfirmed) {
			try {
				...
				if (credit.items && credit.items.length > 0) {
					...
					await db.from("credit_logs").delete().eq("id", credit.id);
				...
				await db.from("credit_logs").delete(credit.id);

				toast.success(
					`Credit record for "${credit.customerName}" has been cancelled and items restored to inventory.`,
					{
						duration: 4000,
						position: "top-right",
					},
