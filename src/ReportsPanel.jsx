...
export default function ReportsPanel({ onClose }) {
  ...
  const creditLogs = useLiveQuery(() => db.creditLog.toArray(), []) || [];
    ...
    last30DaysSales.forEach(sale => {
      ...
      items.forEach(item => {
        if (!productSales30Days[item.productId]) {
          productSales30Days[item.productId] = 0;
        }
        productSales30Days[item.productId] += item.quantity || 0;
      });
    });

    // Calculate daily average
    const dailyAverages = {};
    Object.keys(productSales30Days).forEach(productId => {
      dailyAverages[productId] = productSales30Days[productId] / numberOfDays;
    });
  ...
  const generateReorderSuggestions = () => {
    ...
    inventory.forEach(product => {
      ...
      if (avgDailySales > 0) {
        ...
        const suggestedQuantity = Math.ceil(avgDailySales * desiredDays) - currentStock;
        ...
        suggestions.push({
          ...
          estimatedCost: 10 * (product.costPrice || 0),
          priority: 'unknown',
          urgencyMessage: '❓ OUT OF STOCK - No recent sales data, consider ordering minimum quantity',
          sellingPrice: product.sellingPrice,
          costPrice: product.costPrice
        });
      }
  ...
  const getFilteredSales = () => {
    ...
    if (dateRange === 'today') {
      ...
      filtered = sales.filter(sale => new Date(sale.timestamp).toISOString().split('T')[0] === today);
      ...
      filtered = sales.filter(sale => sale.timestamp >= start && sale.timestamp <= end);
    }
  ...
  filteredSales.forEach(sale => {
    ...
    items.forEach(item => {
      ...
      productSales[productId].revenue += subtotal || 0;
    });
  });
  ...
  const totalRetailValue = inventory.reduce((sum, item) => sum + ((item.sellingPrice || 0) * (item.stock || 0)), 0);
  const potentialProfit = totalRetailValue - totalInventoryValue;
  
  // Credit statistics
  const totalCreditAmount = creditLogs.reduce((sum, credit) => sum + (credit.amount || 0), 0);
  const totalPaidAmount = creditLogs.reduce((sum, credit) => sum + (credit.paidAmount || 0), 0);
  const totalPendingAmount = totalCreditAmount - totalPaidAmount;
  const overdueCredits = creditLogs.filter(credit => 
  ...
  const exportToCSV = (type) => {
    ...
    if (type === 'sales') {
      ...
      data = inventory.map(item => ({
        ...
        'Cost Price': item.costPrice?.toFixed(2) || '0.00',
        'Selling Price': item.sellingPrice?.toFixed(2) || '0.00',
        'Stock': item.stock || 0,
        'Total Value': ((item.costPrice || 0) * (item.stock || 0)).toFixed(2)
      }));
  ...
  const totalReorderCost = suggestedOrder.reduce((sum, item) => sum + item.estimatedCost, 0);
  const criticalItems = suggestedOrder.filter(item => item.priority === 'critical').length;
  const highPriorityItems = suggestedOrder.filter(item => item.priority === 'high').length;
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-4">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-white/80 text-sm mt-1">Monitor sales, inventory, and credit performance</p>
            </div>
            <div className="flex gap-2 relative">
              ...
              {showExportOptions && (
                <div className="absolute right-0 top-12 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 min-w-[180px]">
                  <button
                    onClick={() => { exportToCSV('sales'); setShowExportOptions(false); }}
                    className="block w-full text-left px-4 py-2 hover:bg-gray-50 text-sm"
                  >
                    📊 Export Sales Report
                  </button>
                  <button
                    onClick={() => { exportToCSV('inventory'); setShowExportOptions(false); }}
                    className="block w-full text-left px-4 py-2 hover:bg-gray-50 text-sm"
                  >
                    📦 Export Inventory Report
                  </button>
                  <button
                    onClick={() => { exportToCSV('reorder'); setShowExportOptions(false); }}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                <div className="flex flex-wrap gap-4 items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="font-semibold text-gray-700">Reorder Alert Threshold:</span>
                  ...
                  <div className="text-sm text-gray-500">
                    💡 Items with less than {reorderThreshold} days of stock will appear here
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-4 text-white">
                  ...
                  <p className="text-xs text-red-100 mt-1">Will run out in 1-2 days</p>
                </div>
                <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-4 text-white">
                  ...
                  <p className="text-xs text-orange-100 mt-1">Low stock, reorder soon</p>
                </div>
                <div className="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl p-4 text-white">
                  <p className="text-yellow-100 text-sm">Total Items to Reorder</p>
                  <p className="text-2xl font-bold mt-1">{suggestedOrder.length}</p>
                  <p className="text-xs text-yellow-100 mt-1">Products needing restock</p>
                </div>
                <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-4 text-white">
                  <p className="text-blue-100 text-sm">Estimated Reorder Cost</p>
                  <p className="text-2xl font-bold mt-1">₱{totalReorderCost.toFixed(2)}</p>
                  <p className="text-xs text-blue-100 mt-1">Based on suggested quantities</p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                ...
                {suggestedOrder.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 font-medium">No items need reordering at this time</p>
                    <p className="text-xs text-gray-400 mt-1">All products have sufficient stock levels</p>
                  </div>
                ) : (
                      ...
                      >
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-red-600 font-medium mt-1">{item.urgencyMessage}</p>
                            {item.barcode && (
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-xl font-bold text-blue-600">₱{item.estimatedCost.toFixed(2)}</p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm mt-3">
                          <div>
                            ...
                            <p className={`font-bold text-lg ${item.currentStock === 0 ? 'text-red-600' : 'text-gray-800'}`}>
                              {item.currentStock} units
                            </p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Daily Sales Avg</p>
                            <p className="font-semibold text-gray-800">{item.avgDailySales} units/day</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Days of Stock Left</p>
                            <p className={`font-bold ${
                              item.daysOfStock <= 2 ? 'text-red-600' :
                              item.daysOfStock <= 5 ? 'text-orange-600' :
                              'text-yellow-600'
                            }`}>
                          ...
                          <div>
                            ...
                            <p className="font-bold text-green-600">{item.suggestedQuantity} units</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Cost per Unit</p>
                            <p className="text-gray-700">₱{(item.costPrice || 0).toFixed(2)}</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-3">
                          <div className="flex justify-between text-xs mb-1">
                            ...
                            <span>{Math.min((item.currentStock / (item.suggestedQuantity + item.currentStock)) * 100, 100).toFixed(0)}%</span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div 
                              ...
                              style={{ width: `${Math.min((item.currentStock / (item.suggestedQuantity + item.currentStock)) * 100, 100)}%` }}
                            />
                          </div>
                        </div>
                      </div>
              ...
              {suggestedOrder.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  <h3 className="font-bold text-gray-800 mb-3">📊 Reorder Summary</h3>
                      ...
                      <tbody>
                        {suggestedOrder.map(item => (
                          <tr key={item.id} className="border-b border-gray-100">
                            ...
                            <td className="py-2 text-right">{item.avgDailySales}</td>
                            ...
                            <td className="py-2 text-right font-semibold text-green-600">{item.suggestedQuantity}</td>
                            <td className="py-2 text-right">₱{item.estimatedCost.toFixed(2)}</td>
                          </tr>
                        ))}
                        <tr className="bg-gray-50 font-bold">
                          <td colSpan="4" className="py-2 text-right">Total Estimated Cost:</td>
                          <td className="py-2 text-right text-blue-600">₱{totalReorderCost.toFixed(2)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-4 text-white">
                  ...
                  <p className="text-2xl font-bold mt-1">₱{averageTransaction.toFixed(2)}</p>
                </div>
                <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-4 text-white">
                  <p className="text-orange-100 text-sm">Daily Average</p>
                  <p className="text-2xl font-bold mt-1">₱{(totalSales / Math.max(Object.keys(dailySales).length, 1)).toFixed(2)}</p>
                </div>
              </div>
              ...
              {Object.keys(dailySales).length > 0 && (
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  <h3 className="font-bold text-gray-800 mb-3">📈 Daily Sales Trend</h3>
                  <div className="space-y-2">
                    {Object.entries(dailySales).slice(-7).reverse().map(([date, data]) => (
                      <div key={date}>
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            ...
                            style={{ width: `${Math.min((data.total / totalSales) * 100, 100)}%` }}
                          />
                        </div>
                      </div>
                    ))}
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                ...
                {topProducts.length === 0 ? (
                  <p className="text-center text-gray-500 py-8">No sales data available for the selected period</p>
                ) : (
                      ...
                      <tbody>
                        {filteredSales.slice(0, 10).map(sale => (
                          <tr key={sale.id} className="border-b border-gray-100">
                            <td className="py-2">{new Date(sale.timestamp).toLocaleString()}</td>
                            <td className="py-2">#{sale.id}</td>
                            <td className="py-2 text-right font-semibold">₱{(sale.total || 0).toFixed(2)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          {activeTab === 'inventory' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-4 text-white">
                  <p className="text-green-100 text-sm">Inventory Value (Cost)</p>
                  <p className="text-2xl font-bold mt-1">₱{totalInventoryValue.toFixed(2)}</p>
                </div>
                <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-4 text-white">
                  <p className="text-purple-100 text-sm">Retail Value</p>
                  <p className="text-2xl font-bold mt-1">₱{totalRetailValue.toFixed(2)}</p>
                </div>
                <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-4 text-white">
                  <p className="text-orange-100 text-sm">Potential Profit</p>
                  <p className="text-2xl font-bold mt-1">₱{potentialProfit.toFixed(2)}</p>
                </div>
              </div>
              ...
              {(lowStockItems.length > 0 || outOfStockItems.length > 0) && (
                <div className="bg-yellow-50 border-l-4 border-yellow-400 rounded-xl p-4">
                  <h3 className="font-bold text-yellow-800 mb-2">⚠️ Stock Alerts</h3>
                  {lowStockItems.length > 0 && (
                    <p className="text-sm text-yellow-700">• {lowStockItems.length} product(s) running low on stock (&lt;5 units)</p>
                  )}
                  {outOfStockItems.length > 0 && (
                    <p className="text-sm text-red-700">• {outOfStockItems.length} product(s) out of stock</p>
                  )}
                </div>
              )}
              ...
              {/* Stock Distribution */}
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                <h3 className="font-bold text-gray-800 mb-3">📊 Stock Distribution</h3>
                <div className="space-y-2">
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      <span>In Stock (&gt;5)</span>
                      <span>{inventory.filter(i => i.stock > 5).length} products</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div className="bg-green-500 rounded-full h-2" style={{ width: `${(inventory.filter(i => i.stock > 5).length / inventory.length) * 100}%` }} />
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div className="bg-yellow-500 rounded-full h-2" style={{ width: `${(lowStockItems.length / inventory.length) * 100}%` }} />
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div className="bg-red-500 rounded-full h-2" style={{ width: `${(outOfStockItems.length / inventory.length) * 100}%` }} />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                <h3 className="font-bold text-gray-800 mb-3">📋 Complete Inventory</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="border-b">
                      <tr>
                    ...
                    <tbody>
                      {inventory.map(item => (
                        <tr key={item.id} className="border-b border-gray-100">
                          <td className="py-2 font-medium">{item.name}</td>
                          <td className="py-2 text-xs font-mono">{item.barcode || 'N/A'}</td>
                          <td className="py-2 text-right">₱{(item.costPrice || 0).toFixed(2)}</td>
                          <td className="py-2 text-right">₱{(item.sellingPrice || 0).toFixed(2)}</td>
                          <td className={`py-2 text-right font-semibold ${item.stock === 0 ? 'text-red-600' : item.stock < 5 ? 'text-yellow-600' : 'text-green-600'}`}>
                            {item.stock || 0}
                          </td>
                          <td className="py-2 text-right">₱{((item.costPrice || 0) * (item.stock || 0)).toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-4 text-white">
                  ...
                  <p className="text-2xl font-bold mt-1">₱{totalCreditAmount.toFixed(2)}</p>
                </div>
                <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-4 text-white">
                  <p className="text-green-100 text-sm">Total Paid</p>
                  <p className="text-2xl font-bold mt-1">₱{totalPaidAmount.toFixed(2)}</p>
                </div>
                <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-4 text-white">
                  <p className="text-red-100 text-sm">Pending Balance</p>
                  <p className="text-2xl font-bold mt-1">₱{totalPendingAmount.toFixed(2)}</p>
                </div>
                <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-4 text-white">
              ...
              {totalCreditAmount > 0 && (
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  <h3 className="font-bold text-gray-800 mb-3">📊 Collection Rate</h3>
                  <div className="flex flex-col sm:flex-row items-center gap-6">
                    <div className="relative w-32 h-32">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <p className="text-2xl font-bold text-green-600">{((totalPaidAmount / totalCreditAmount) * 100).toFixed(1)}%</p>
                      </div>
                    </div>
                    <div className="flex-1">
                      <p className="text-sm text-gray-600">Collection efficiency based on total credits issued</p>
                      <div className="mt-2 space-y-1">
                        <p className="text-sm">✅ Paid: ₱{totalPaidAmount.toFixed(2)}</p>
                        <p className="text-sm">⏳ Pending: ₱{totalPendingAmount.toFixed(2)}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* Active Credits */}
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                <h3 className="font-bold text-gray-800 mb-3">📋 Active Credits</h3>
                {creditLogs.filter(c => c.status !== 'paid').length === 0 ? (
                  <p className="text-center text-gray-500 py-8">No active credits</p>
                ) : (
                  <div className="space-y-3">
                    {creditLogs.filter(c => c.status !== 'paid').slice(0, 10).map(credit => {
                      const remaining = (credit.amount || 0) - (credit.paidAmount || 0);
                      const isOverdue = credit.dueDate && new Date(credit.dueDate) < new Date();
                      
                      return (
                        <div key={credit.id} className={`border rounded-lg p-3 ${isOverdue ? 'border-red-200 bg-red-50' : 'border-gray-200'}`}>
                          <div className="flex justify-between items-start mb-2">
                            <div>
                              <p className="font-semibold text-gray-800">{credit.customerName}</p>
                              <p className="text-xs text-gray-500">Due: {credit.dueDate ? new Date(credit.dueDate).toLocaleDateString() : 'No due date'}</p>
                            </div>
                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                              credit.status === 'partial' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'
                            }`}>
                              {credit.status === 'partial' ? 'Partial' : 'Unpaid'}
                            </span>
                          </div>
                          <div className="grid grid-cols-3 gap-2 text-sm">
                            <div>
                              <p className="text-xs text-gray-500">Total</p>
                              <p className="font-semibold">₱{(credit.amount || 0).toFixed(2)}</p>
                            </div>
                            <div>
                              <p className="text-xs text-gray-500">Paid</p>
                              <p className="font-semibold text-green-600">₱{(credit.paidAmount || 0).toFixed(2)}</p>
                            </div>
                            <div>
